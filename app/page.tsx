...
export default function CafeMuseum() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // For mobile, use simpler animations to prevent white space issues
      if (isMobile) {
        // Simple fade-in only for mobile to prevent layout issues
      ...
      // Signature drinks scroll animation with minimal displacement
      gsap.fromTo(
        ".drink-card",
        {
          opacity: 0,
          y: isMobile ? 30 : 50, // Reduced from 50/100
          scale: isMobile ? 0.98 : 0.95, // Less scaling to prevent layout shift
        },
        {
          opacity: 1,
          y: 0,
      ...
      // About section animation with minimal displacement
      gsap.fromTo(
        ".about-content",
        {
          opacity: 0,
          x: isMobile ? -30 : -50, // Reduced from -50/-100
        },
        {
          opacity: 1,
          x: 0,
      ...
      // Menu cards animation with reduced motion on mobile
      gsap.fromTo(
        ".menu-card",
        {
          opacity: 0,
      ...
      // Events section animation with responsive distance
      gsap.fromTo(
        ".event-item",
        {
          opacity: 0,
      ...
      // Instagram grid animation with reduced effects on mobile
      gsap.fromTo(
        ".instagram-item",
        {
          opacity: 0,
      ...
      // Text reveal animations for section headers with responsive settings
      gsap.fromTo(
        ".section-header",
        {
          opacity: 0,
          y: isMobile ? 20 : 30,
          clipPath: isMobile ? "inset(0 0 50% 0)" : "inset(0 0 100% 0)",
        },
        {
          opacity: 1,
          y: 0,
